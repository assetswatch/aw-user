  ...
  const getUsers = ({
    ...
    if (Object.keys(formErrors).length > 0) {
      ...
      objParams = {
        ...
        InviterProfileTypeId: loggedinprofiletypeid, //config.userProfileTypes.Owner,
    ...
    navigate(
      routeNames.connectionssendinvite.path +
        `?p=${Object.keys(config.userProfileTypes).find(
          (k) => config.userProfileTypes[k] === config.userProfileTypes.Owner
        )}`
    );
  };
              ...
              <div className="tabw100 tab-action shadow rounded bg-white">
                <ul className="nav-tab-line list-color-secondary d-table mb-0 d-flex box-shadow">
                  {loggedinprofiletypeid == config.userProfileTypes.Owner ? (
                    <>
                      <li className="active">Owners</li>
                      <li onClick={navigateToAgents}>Agents</li>
