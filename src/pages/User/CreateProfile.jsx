...
const CreateProfile = () => {
  ...
  const getCreateProfileTypeId = () => {
    const createProfileTypeKey = Object.keys(config.userProfileTypes).find(
      (key) => key.toLowerCase() === ProfileType.toLowerCase()
    );

    return createProfileTypeKey
      ? config.userProfileTypes[createProfileTypeKey]
      : 0;
  };

  useEffect(() => {
    if (getCreateProfileTypeId() == 0) {
      ...
      .then((response) => {
        ...
        if (objResponse.StatusCode === 200) {
          ...
          if (checkObjNullorEmpty(details) == false) {
            ...
            if (getCreateProfileTypeId() === config.userProfileTypes.Agent) {
  ...
  const onCreate = (e) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      objBodyParams.append("ProfileTypeId", getCreateProfileTypeId());
      ...
      objBodyParams.append(
        "ProfileCategoryId",
        getCreateProfileTypeId() === config.userProfileTypes.Agent
        ...
        .then((response) => {
          ...
          if (objResponse.StatusCode === 200) {
            if (objResponse.Data != null && objResponse.Data?.ProfileId > 0) {
              Toast.success(AppMessages.CreateProfileSuccess);
              returntoProfilesPage();
            } else {
