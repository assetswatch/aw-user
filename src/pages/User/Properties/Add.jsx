...
const Add = () => {
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      for (let od = 0; od <= ownerdivs.length - 1; od++) {
        ...
        objBodyParams.append(
          `Owners[${od}].Name`,
          ownerFormData[`txtname${od + 1}`]
        );
        objBodyParams.append(
          `Owners[${od}].Email`,
          ownerFormData[`txtemail${od + 1}`]
        );
        objBodyParams.append(
          `Owners[${od}].Mobile`,
          ownerFormData[`txtmobile${od + 1}`]
        );
        objBodyParams.append(
          `Owners[${od}].AddressOne`,
          ownerFormData[`txtaddressone${od + 1}`]
        );
        objBodyParams.append(
          `Owners[${od}].AddressTwo`,
          ownerFormData[`txtaddresstwo${od + 1}`]
        );
        objBodyParams.append(
          `Owners[${od}].CountryId`,
          ownerFormData[`ddlcountry${od + 1}`]
        );
        objBodyParams.append(
          `Owners[${od}].StateId`,
          ownerFormData[`ddlstate${od + 1}`]
        );
        objBodyParams.append(
          `Owners[${od}].CityId`,
          ownerFormData[`ddlcity${od + 1}`]
        );
        objBodyParams.append(
          `Owners[${od}].Zip`,
          ownerFormData[`txtzip${od + 1}`]
        );
      }
      ...
      objBodyParams.append("AddressTwo", formData.txtaddresstwo);
      objBodyParams.append(
        "CountryId",
      ...
      objBodyParams.append("Advance", formData.txtadvance);
      objBodyParams.append(
        "AccessTypeId",
      ...
      objBodyParams.append(
        "Bedrooms",
        parseInt(setSelectDefaultVal(selectedBedRooms))
      );
      objBodyParams.append(
        "Bathrooms",
        parseInt(setSelectDefaultVal(selectedBathRooms))
      );
      objBodyParams.append("BrokerPercentage", formData.txtbrokerpercentage);
      ...
      axiosPost(`${config.apiBaseUrl}${ApiUrls.addAsset}`, objBodyParams, {
        ...
        .finally(() => {
          ...
          apiReqResLoader("btnSave", "Save", "completed");
        });

      // objFormData.append("assetInfo.Info", JSON.stringify({ Title1: "*&" }));
      // console.log(objFormData);
      // axiosPost(`${config.apiBaseUrl}${ApiUrls.addAsset}`, objFormData, {
      //   "Content-Type": "multipart/form-data",
      // })
      //   .then((response) => {
      //     let objResponse = response.data;
  ...
  console.log(assetsAppConfigList);
  return (
    <>
      {SetPageLoaderNavLinks()}
        ...
        <div className="container">
          <div className="row">
            <div className="col-12">
              <h5 className="mb-4 down-line">Add / Edit Property</h5>
              <div className="row">
                            ...
                            <div className="row">
                              ...
                              <div className="col-md-12 text-right px-0">
                                <button
                                  ...
                                  onClick={() => removeOwnersDiv(div.id)}
                                >
                                  Remove
                                </button>
                              </div>
                    ...
                    <div className="full-row px-3 py-4 mt-20 bg-white box-shadow rounded">
                      <div className="container-fluid">
                        <div className="row form-action flex-center">
                          <div className="col-md-6 px-0 form-error"></div>
